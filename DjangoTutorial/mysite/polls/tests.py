...
def create_question_with_choices(question_text="TestText", days=0,
                                 choice_text='Test', votes=0):
...
def create_question_without_choices(question_text="TestText", days=0):
    time = timezone.now() + datetime.timedelta(days=days)
...
class QuestionIndexViewTests(TestCase):
    ...
    def test_questions_without_choices(self):
        create_question_without_choices(question_text="Question without choices1.", days=-1)
        response = self.client.get(reverse('polls:index'))
        self.assertQuerysetEqual(response.context['latest_question_list'], [])

    def test_questions_with_choices(self):
        create_question_with_choices(question_text="Question with choices1.", days=-1)
        response = self.client.get(reverse('polls:index'))
        self.assertQuerysetEqual(
            response.context['latest_question_list'],
            ['<Question: Question with choices1.>']
        )
...
class QuestionDetailViewTests(TestCase):
    ...
    def test_question_without_choices(self):
        question = create_question_without_choices(question_text='Past Question.', days=-5)
